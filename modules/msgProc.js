...
exports.proc = (tri, msg, cb) => {
  ...
  if (msg.channel.id !== '616539086055800842') return
  if (!msg.content.startsWith('트리튬,') && !msg.content.startsWith('3,') && !msg.content.startsWith('트,')) return

  const msgReals = msg.content.split(',')[1].trim()

  commandList.forEach((command) => {
    command.keywords.forEach((keyword) => {
      if (msgReals === keyword) command.module.run(tri, msg, (cmdErr) => {
        if (cmdErr) cb(cmdErr)
        else cb()
        return
      })
  ...
  superagent.get('https://open-korean-text-api.herokuapp.com/extractPhrases?text=' + encodeURI(msgReals), (err, res) => {
    if (err) cb(err)
    else {
      ...
      commandList.forEach((command) => {
        command.keywords.forEach((keyword) => {
          if (result.includes(keyword)) command.module.run(tri, msg, (cmdErr) => {
            if (cmdErr) cb(cmdErr)
            else cb()
            return
          })
    ...
    dialogClient.detectIntent({ session: dialogClient.sessionPath('tritium-kkbvva', msg.author.id), queryInput: { text: { text: msgReals, languageCode: 'ko-KR' } } }).then((res) => {
